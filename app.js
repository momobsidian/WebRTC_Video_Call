...
async function initLocalStream() {
    try {
        ...
        // Check if mediaDevices is supported (usually requires HTTPS on mobile)
        if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
            throw new Error('MediaDevices API not available. On mobile, this requires HTTPS or localhost.');
        }
        ...
        // Check if there are multiple cameras to show the switch button
        ...
        let errorMsg = 'Could not access camera/microphone. Please check your browser settings and grant permissions.';
        if (error.message.includes('HTTPS')) {
            errorMsg = 'Camera access is blocked by your mobile browser because the site is not secure. You MUST use HTTPS or localhost to use the camera on mobile.';
        } else if (error.name === 'NotAllowedError' || error.name === 'PermissionDeniedError') {
            errorMsg = 'Permission to use camera/microphone was denied. You must go to your browser settings for this site and allow access, then try again.';
        }
        
        window.showModal('Permission Required', errorMsg, 'fa-video-slash');
        return false;
    }
}
...
window.switchCamera = async function() {
    ...
    try {
        debugLog(`Switching camera to ${currentFacingMode}...`, 'info');
        ...
        // Apply muted state to new stream if user explicitly disabled video
        const videoBtn = document.getElementById('videoBtn');
        if (videoBtn && videoBtn.classList.contains('muted')) {
        ...
        debugLog('✓ Camera switched successfully', 'success');
        
        // Add visual feedback to button
        const btn = document.getElementById('switchCameraBtn');
        if (btn) {
            btn.classList.add('captured');
            setTimeout(() => btn.classList.remove('captured'), 500);
        }
    } catch (error) {
        debugLog(`✗ Error switching camera: ${error.message}`, 'error');
        window.showModal('Camera Error', 'Could not switch camera. Another app might be using it.', 'fa-video-slash');
